...
import {fetchTopAiringAnime, fetchairingAnime} from '../../../utils/functions';
...
const Airing = ({navigation, routes}) => {
  // const [airing, setAiring] = useState([])
  const [filterairing, setFilterAiring] = useState([]);
  ...
  const {
    ...
    queryKey: ['airing', pages.currentPage, refreshing],
    queryFn: () => fetchairing(pages.currentPage),
  ...
  const renderItem = useCallback(
    ...
    },[airing, filterairing, filter, isLoading, refreshing],
  );
  ...
  return (
    <View style={styles.container}>
      {isLoading ? (
        <FlatList
          data={[1, 2, 3, 4, 5, 6, 7]}
          keyExtractor={(item, index) => `${item}-${index}-AiringStackSkeleton`}
        ...
        <FlatList
          data={filterairing.length > 0 ? filterairing : airing}
          keyExtractor={(item, index) => `${item.animeID}-${index}-airingStackFlatList`}
