...
const BottomSheet2 = props => {
  ...
  const MAX_TRANSLATE_Y = VideoPlayer.fullscreen
    ? width * 0.03
    : SCREEN_HIEGHT * max_Trans_Y;
  const MAX_HEIGHT = VideoPlayer.fullscreen
    ? width - MAX_TRANSLATE_Y - 20
    : SCREEN_HIEGHT - MAX_TRANSLATE_Y - 10;
  const BORDER_RADIUS = borderRadius;
  const END_POINT = VideoPlayer.fullscreen
    ? width * endPoint
    : SCREEN_HIEGHT * endPoint;
  const SNAP_POINT = VideoPlayer.fullscreen
    ? width * 0.2
    : SCREEN_HIEGHT * snapPoint;
  const LEFT = VideoPlayer.fullscreen ? SCREEN_HIEGHT * 0.5 : 0;
  // const RIGHT= 0
  const RIGHT= VideoPlayer.fullscreen?SCREEN_HIEGHT*0.2:0
        ...
        {VideoPlayer.fullscreen ? (
          <View
            style={[
              ...
              <View style={[styles.container, {width:"100%", height:"100%",paddingTop:20,borderRadius:borderRadius}]}>
              <>{children}</>
